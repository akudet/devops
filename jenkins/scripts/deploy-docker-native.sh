...
DOCKER_RUN="docker run --name $DEPLOY_NAME -d --restart=always -v run_env:/run/secrets/run_env:ro --network $DEPLOY_ENV -e SPRING_PROFILES_ACTIVE=$DEPLOY_ENV $DOCKER_IMAGE"
